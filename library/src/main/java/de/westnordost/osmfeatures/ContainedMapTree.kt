...
@JvmOverloads constructor(maps: Collection<Map<K, V>>, maxDepth: Int = 4, minContainerSize: Int = 4) {
    ...
    private class Node<K, V>(
        ...
        fun getAll(map: Map<K, V>): List<Map<K, V>> {
            ...
            if (maps != null) {
                for (m in maps) {
                    if (mapContainsAllEntries(map, m.entries)) {
                        result.add(m)
                    }
                }
            }
    ...
    companion object {
        ...
        private fun <K, V> getMapsByKeyValue(key: K, maps: Collection<Map<K, V>>): Map<V, MutableList<Map<K, V>>> {
